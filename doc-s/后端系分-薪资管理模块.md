...
![image.png](https://alidocs.oss-cn-zhangjiakou.aliyuncs.com/res/4maOgX36mb88KlWN/img/64f9a571-e23c-40a5-ae97-50a8eceb4ce7.png)
...
| **ChangeType** | 1~5 | SALARY_ADJUST(1) 调薪 / ACCOUNT_CHANGE(2) 账套变更 / BASE_ADJUST(3) 基数调整 / REGULAR_ADJUST(4) 转正调薪 / TRANSFER\_ADJUST(5) 调岗调薪 |

### 薪资计算引擎架构（策略模式）

![image.png](https://alidocs.oss-cn-zhangjiakou.aliyuncs.com/res/4maOgX36mb88KlWN/img/2c5800f4-f1ed-4a68-b3eb-6e22c7db0ce6.png)
...
![image.png](https://alidocs.oss-cn-zhangjiakou.aliyuncs.com/res/4maOgX36mb88KlWN/img/bda85292-c85d-4a5f-928b-e571b25152da.png)
...
![image.png](https://alidocs.oss-cn-zhangjiakou.aliyuncs.com/res/4maOgX36mb88KlWN/img/a8cf01d5-334a-4cf7-9c41-5e7aaa1147e9.png)
...
| scope\_type | Integer | 否 | 适用范围类型：1=部门 2=职位 3=职级 |
...
| --- | --- | --- | --- |
| name | String | 是 | 账套名称 |
| scope\_type | Integer | 是 | 适用范围类型：1=部门 2=职位 3=职级 |
| scope\_ids | Long\[\] | 是 | 适用范围ID集合 |
| effective\_date | Date | 是 | 生效日期 |
...
| --- | --- | --- | --- |
| name | String | 是 | 项目名称 |
| item\_type | Integer | 是 | 项目类型：1~6 |
| formula | String | 否 | 计算公式/规则描述 |
| sort\_order | Integer | 是 | 排序序号 |
| is\_taxable | Integer | 是 | 是否计入个税：0=否 1=是 |
...
| PUT | `/api/v1/salary-accounts/{id}/items/sort` | 调整排序（body: \`{"item\_ids": \[3,1,2\]}\`） |
...
| 参数 | 类型 | 必填 | 描述 |
| --- | --- | --- | --- |
| account\_id | Long | 是 | 适用账套ID |
| base\_salary | BigDecimal | 是 | 基本工资（元） |
| allowance\_base | BigDecimal | 是 | 津贴基数（元） |
| social_security_base | BigDecimal | 是 | 社保基数（元） |
| housing_fund_base | BigDecimal | 是 | 公积金基数（元） |
| performance\_base | BigDecimal | 否 | 绩效基数（元） |
| effective\_date | Date | 是 | 生效日期 |
...
| 参数 | 类型 | 必填 | 描述 |
| --- | --- | --- | --- |
| salary\_month | String | 否 | 核算月份，格式 yyyy-MM |
...
| 参数 | 类型 | 必填 | 描述 |
| --- | --- | --- | --- |
| salary\_month | String | 是 | 核算月份，格式 yyyy-MM |
...
| 参数 | 类型 | 必填 | 描述 |
| --- | --- | --- | --- |
| detail\_id | Long | 是 | 薪资明细ID |
| adjustment\_amount | BigDecimal | 是 | 调整金额，正数为补发，负数为扣减 |
| adjustment\_reason | String | 否 | 调整原因 |
...
| 参数 | 类型 | 必填 | 描述 |
| --- | --- | --- | --- |
| verify\_type | Integer | 是 | 验证方式：1=短信验证码 2=登录密码 |
| verify\_code | String | 是 | 验证码或密码 |
...
| 无薪资档案 | 新员工无 employee\_salary 记录 | BLOCKED(3) | 红色阻断，无法计算 |
...
**后端总预估工期**：约 14.5 个工作日 **前端总预估工期**：约 13.5 个工作日 **总预估工期**：约 24 个工作日（前后端并行开发）