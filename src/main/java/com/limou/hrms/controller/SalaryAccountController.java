...
public class SalaryAccountController {
    ...
    @GetMapping("/{id}")
    public BaseResponse<SalaryAccountVO> getAccount(
    ...
    @ApiOperation("删除账套")
    @DeleteMapping("/{id}")
    public BaseResponse<Boolean> deleteAccount(
            @ApiParam("账套ID") @PathVariable Long id) {
        if (id == null || id <= 0) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
    ...
    @GetMapping("/{id}/items")
    public BaseResponse<List<SalaryItemVO>> listItems(
            @ApiParam("账套ID") @PathVariable Long id) {
        if (id == null || id <= 0) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
    ...
    public BaseResponse<Boolean> deleteItem(
            @ApiParam("工资项目ID") @PathVariable Long itemId) {
